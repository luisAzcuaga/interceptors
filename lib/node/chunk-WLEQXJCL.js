...
var _chunkV3JB6OXOjs = require('./chunk-V3JB6OXO.js');
...
function createResponse(message) {
  const responseBodyOrNull = _chunkV3JB6OXOjs.RESPONSE_STATUS_CODES_WITHOUT_BODY.includes(
...
function createRequest(clientRequest) {
  ...
  const fetchRequest = new Request(clientRequest.url, {
    method: _chunkV3JB6OXOjs.FORBIDDEN_REQUEST_METHODS.includes(method) ? `UNSAFE-${method}` : method,
    headers,
  ...
  respondWith(mockedResponse) {
    ...
    isResponseStreamFinished.then(() => {
      ...
      if (this.method === "CONNECT") {
        ...
        const socket = new (0, _stream.Duplex)({
          read() {
          },
...
function getUrlByRequestOptions(options) {
  ...
  const path = options.path ? (
     ...
     */
    options.method === "CONNECT" ? DEFAULT_PATH : (
      /**
...
function normalizeClientRequestArgs(defaultProtocol, ...args) {
  ...
  if (typeof args[0] === "string") {
    ...
    if (args[0] === "") {
      logger5.info("empty URL string, treating as options-only request");
